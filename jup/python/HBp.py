...
def setup_options(argv):
    ...
    functions = []
    try:
        opts, args = getopt.getopt(argv,"ahi:no:s:",["ifile=","ofile=","strmatch="])
    ...
    if not path_exists(outputpath):
        if path_writable(outputpath):
            ...
            if outputpath[0] != '.' and outputpath[1] != '/':
                choice = str(input('Can I use ./'+outputpath+'? (y/n)'))
            if choice == 'y':
        ...
        choice = str(input("WARNING: file "+outputpath+" exists: continue? (y/n)"))
        if choice == 'y':
            pass
        else:
            sys.exit(42)
...
def data_analysis_on_news(argdict):
    ...
    outname = outname[:outname.find("csv")-1]+'_news'+outname[outname.find('csv')-1:]
...
def data_analysis_on_agents(argdict):
    ...
    outname = outname[:outname.find("csv")-1]+'_agents'+outname[outname.find('csv')-1:]
            ...
            for i, e in enumerate(row):
                ...
                if row[i] != row[i-1] and i != 0:
                    if row[i] == '0':
                        ...
                        isdown =False
    import numpy as np
    upunique,   upcounts  = np.unique(temp_time_dict['up'], return_counts=True)
    downunique, downcounts= np.unique(temp_time_dict['down'], return_counts=True)
    with open(outname, 'w') as writefile:
        csvwriter = csv.writer(writefile, delimiter=',')
