...
class ClusteringScheduler(AgentScheduler):
    """
    ...
    def __init__(self, number, myWorldState, agType, *args, **kwargs):
        ...
        self.filename = common.project.replace(
            "src", 'tmp/cls_log_temp.%s.txt' % os.getpid())
        ...
        printHeader(
            self.writer,
            firstline=['# clusteringlog'],
            lastline=['time', 'agent', 'clustering'])
        common.clslog = self
    ...
    def registerEntry(self, date=-1, graph=None, write=True):
        ...
        for key, val in dict(clus).items():
